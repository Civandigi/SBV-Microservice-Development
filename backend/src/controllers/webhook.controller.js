...
class WebhookController {
     ...
     */
    static handleGesuchProcessed = asyncHandler(async (req, res) => {
     ...
     */
    static handleRapporteReady = asyncHandler(async (req, res) => {
        const signature = req.headers['x-webhook-signature'];
        if (!WebhookController.validateSignature(req.body, signature)) {
            return res.status(401).json({
                success: false,
     ...
     */
    static handleWordReady = asyncHandler(async (req, res) => {
        const signature = req.headers['x-webhook-signature'];
        if (!WebhookController.validateSignature(req.body, signature)) {
            return res.status(401).json({
                success: false,
