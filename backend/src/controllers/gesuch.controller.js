...
const uploadGesuch = async (req, res) => {
  ...
  try {
    ...
    console.error('[Gesuch] Upload error:', error);
    res.status(500).json({
      success: false,
...
const updateTeilprojekte = async (req, res) => {
  try {
    ...
    console.error('[Gesuch] Update error:', error);
    res.status(500).json({ 
      success: false,
      error: 'Fehler beim Aktualisieren der Teilprojekte'
    });
  }
};

// Rapporte aus Gesuch erstellen
const createRapporte = async (req, res) => {
  try {
    const { gesuchId } = req.params;
    ...
    console.error('[Gesuch] Create rapporte error:', error);
    res.status(500).json({
      success: false,
      error: 'Fehler beim Erstellen der Rapporte',
      details: error.message
    });
  }
};
...
const getAllGesuche = async (req, res) => {
  try {
    ...
    console.error('[Gesuch] Get all error:', error);
    res.status(500).json({
      success: false,
      error: 'Fehler beim Abrufen der Gesuche'
    });
  }
};

// Einzelnes Gesuch mit Details
const getGesuchDetails = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    console.error('[Gesuch] Get details error:', error);
    res.status(500).json({
      success: false,
      error: 'Fehler beim Abrufen der Gesuch-Details'
    });
  }
};
